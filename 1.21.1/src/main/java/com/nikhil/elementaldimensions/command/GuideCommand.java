...
public class GuideCommand {
    ...
    private static int executeGuide(CommandContext<ServerCommandSource> context) {
        ...
        try {
            ...
            lore.add(Text.literal("âœ¦ Inferno Realm - Pyros").formatted(Formatting.RED));
            lore.add(Text.literal("âœ¦ Aquatic Realm - Leviathan").formatted(Formatting.BLUE));
            lore.add(Text.literal("âœ¦ Terran Realm - Goliath").formatted(Formatting.GREEN));
            lore.add(Text.literal("âœ¦ Skybound Realm - Zephyros").formatted(Formatting.WHITE));
            lore.add(Text.literal("âœ¦ Celestial Realm - Primarch").formatted(Formatting.LIGHT_PURPLE));
            ...
            if (success) {
                source.sendFeedback(() -> Text.literal("âœ¦ You have received the Elemental Dimensions Guide!")
