...
/**
 * Generates a simple crystal structure in Terra dimension
 */
public class CrystalCavernFeature extends Feature<DefaultFeatureConfig> {
	...
	public boolean generate(FeatureContext<DefaultFeatureConfig> context) {
				...
				for (int z = -size; z <= size; z++) {
					if (x == -size || x == size || z == -size || z == size) {
						if (y < size + 2 || (x % 2 == 0 && z % 2 == 0)) {
							world.setBlockState(pos.add(x, y, z), ModBlocks.TERRAN_BEDROCK.getDefaultState(), 3);
						}
					}
				}
			}
		...
		world.setBlockState(pos.add(size, 2, 0), ModBlocks.EMERALD_CLUSTER.getDefaultState(), 3);
		world.setBlockState(pos.add(0, 2, -size), ModBlocks.EMERALD_CLUSTER.getDefaultState(), 3);
		world.setBlockState(pos.add(0, 2, size), ModBlocks.EMERALD_CLUSTER.getDefaultState(), 3);
