...
public class DimensionalCompassItem extends Item {
	...
	public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
		...
		if (!world.isClient()) {
			...
			user.sendMessage(Text.literal("Â§6Current Dimension: Â§e" + getDimensionName(dimensionId)), true);

			// Check if player is in the Overworld
			if (dimensionId.equals("minecraft:overworld")) {
				user.sendMessage(Text.literal("Â§7Use this compass in dimensional realms to find portals"), false);
			} else {
				// In a dimension - provide basic guidance
				user.sendMessage(Text.literal("Â§aCompass active! Â§7Portal detection: In development"), false);
			}
		}
