...
function New-AssetDirectories {
    Write-Output "Creating asset directory structure..." -ForegroundColor Cyan
    # Texture directories
    $textureDirs = @(
        "textures\block",
        "textures\item",
    ...
    Write-Output "`nDirectory structure created successfully!" -ForegroundColor Green
}
function Show-MissingAssets {
    Write-Output "`n=== MISSING ASSETS CHECKLIST ===" -ForegroundColor Yellow
    # Block textures
    $blockTextures = @(
        "void_stone.png",
    ...
    Write-Output "`nBLOCK TEXTURES (16x16 PNG):" -ForegroundColor Cyan
    foreach ($texture in $blockTextures) {
        $path = Join-Path $AssetsRoot "textures\block\$texture"
        if (Test-Path $path) {
            Write-Output "  [✓] $texture" -ForegroundColor Green
        } else {
            Write-Output "  [ ] $texture" -ForegroundColor Red
        }
    }
    Write-Output "`nITEM TEXTURES (16x16 PNG):" -ForegroundColor Cyan
    ...
    Write-Output "`nENTITY TEXTURES (64x64+ PNG):" -ForegroundColor Cyan
    ...
    Write-Output "`nMUSIC TRACKS (OGG format):" -ForegroundColor Cyan
    foreach ($dimension in $musicTracks.Keys) {
        Write-Output "  $dimension/:" -ForegroundColor Yellow
        foreach ($track in $musicTracks[$dimension]) {
            $path = Join-Path $AssetsRoot "sounds\music\$dimension\$track"
            if (Test-Path $path) {
                Write-Output "    [✓] $track" -ForegroundColor Green
            } else {
                Write-Output "    [ ] $track" -ForegroundColor Red
            }
        }
    }
    Write-Output "`nAMBIENT SOUNDS (OGG format):" -ForegroundColor Cyan
    foreach ($sound in $ambientSounds) {
        $path = Join-Path $AssetsRoot "sounds\ambient\$sound"
        if (Test-Path $path) {
            Write-Output "  [✓] $sound" -ForegroundColor Green
        } else {
            Write-Output "  [ ] $sound" -ForegroundColor Red
        }
    }
}
function Show-FreesoundSearches {
    Write-Output "`n=== FREESOUND.ORG SEARCH SUGGESTIONS ===" -ForegroundColor Yellow
    Write-Output "Visit https://freesound.org and search for these terms:" -ForegroundColor Cyan
    Write-Output ""
    $searches = @{
        "FIRE/LAVA SOUNDS" = @(
            "fire crackling loop",
    ...
    foreach ($category in $searches.Keys) {
        Write-Output "`n$category" -ForegroundColor Green
        foreach ($search in $searches[$category]) {
            Write-Output "  • $search" -ForegroundColor White
        }
    }
    Write-Output "`n"
    Write-Output "TIP: Filter by 'Creative Commons 0' license for easiest usage!" -ForegroundColor Yellow
    Write-Output "TIP: Download WAV or OGG format (use Audacity to convert if needed)" -ForegroundColor Yellow
}
function Show-TextureSearches {
    Write-Output "`n=== TEXTURE SEARCH SUGGESTIONS ===" -ForegroundColor Yellow
    Write-Output ""
    Write-Output "OpenGameArt.org searches:" -ForegroundColor Cyan
    $ogaSearches = @(
    ...
    Write-Output "`nPixabay.com searches:" -ForegroundColor Cyan
    $pixabaySearches = @(
        "stone texture",
        "crystal texture",
        "dark rock seamless",
    ...
    Write-Output "`nitch.io searches (game-assets/free):" -ForegroundColor Cyan
    $itchSearches = @(
        "pixel art items",
        "minecraft textures",
        "16x16 tileset",
...
function New-AssetCreditsTemplate {
    ...
    if (Test-Path $creditsPath) {
        Write-Output "`nASSET_CREDITS.md already exists. Skipping..." -ForegroundColor Yellow
        return
    }
    ...
    Write-Output "`nCreated ASSET_CREDITS.md template" -ForegroundColor Green
}
# Main execution
if ($Help) {
    Show-Help
...
Write-Output "`n=== NEXT STEPS ===" -ForegroundColor Yellow
Write-Output @"
...
Write-Output "Script complete!" -ForegroundColor Green